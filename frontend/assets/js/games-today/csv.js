...
export async function fetchCSV(path) {
  ...
  try {
    const resolvedPath = path.startsWith("win/")
  ? "https://raw.githubusercontent.com/Clownworldenjoyer76/bet_tracker/main/docs/" + path
  : path;

const cacheBust = resolvedPath.includes("?") ? `&v=${Date.now()}` : `?v=${Date.now()}`;
    const res = await fetch(`${resolvedPath}${cacheBust}`, { cache: "no-store" });
