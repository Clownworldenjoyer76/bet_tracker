...
export async function loadMLB(date) {
  ...
  const [gamesRes, predRes, bookRes, contextRes] = await Promise.all([
    fetchCSV(`win/baseball/mlb/00_intake/games/${date}_games.csv`),
    fetchCSV(`win/baseball/mlb/00_intake/predictions/pred_with_game_id/${date}_MLB.csv`),
    fetchCSV(`win/baseball/mlb/00_intake/sportsbook/${date}_MLB.csv`),
    fetchCSV(`win/baseball/mlb/00_intake/mlb_raw/${date}_game_context.csv`),
  ]);
