...
# --- REPORT BUILDERS (unchanged structure, validation added everywhere) ---
...
def build_spread_outputs(work, league, outdir):
    try:
        ...
        validate_aggregation(df1_src, df1, f"{league} spread_edge_bucket_home_away_summary")
        ...
        validate_aggregation(df2_src, df2, f"{league} spread_edge_bucket_summary")
        ...
        validate_aggregation(df3_src, df3, f"{league} spread_bands_bucket_home_away_summary")
        ...
        validate_aggregation(df4_src, df4, f"{league} spread_bands_bucket_summary")

        df5_src = sp[(sp["side_group"].isin(["HOME","AWAY"])) & (sp["spread_bucket"] != "UNBUCKETED") & (sp["edge_bucket"] != "UNBUCKETED")]
        df5 = aggregate_results(df5_src, ["market","market_type","side_group","spread_bucket","edge_bucket"])
        write_csv(df5, outdir / "spread_summary.csv")
        validate_aggregation(df5_src, df5, f"{league} spread_summary")
