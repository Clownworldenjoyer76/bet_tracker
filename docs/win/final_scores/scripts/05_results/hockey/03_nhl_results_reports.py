...
df["kelly_bucket"] = df["kelly"].apply(kelly_bucket) if "kelly" in df.columns else df["puck_line_bucket"] = df.apply(puck_line_bucket, axis=1)
...
if (df["is_win"].sum() + df["is_loss"].sum()) > 0
else np.nan
],
"units": [df["units"].sum()],
...
columns={"market_type_norm": "market_type"}
).to_csv(BY_MARKET_FILE, index=False)
...
# =========================
# PER-MARKET DETAIL REPORTS
# 4 files per market x 3 markets = 12 files
# edge bucket, odds bucket, kelly bucket, side
# =========================
...
if mdf.empty:
continue
summarize(mdf, ["edge_bucket"]).to_csv(
MARKET_DETAIL_DIR / f"nhl_{slug}_by_edge.csv", index=False
)
summarize(mdf, ["odds_bucket"]).to_csv(
MARKET_DETAIL_DIR / f"nhl_{slug}_by_odds.csv", index=False
)
if "kelly_bucket" in mdf.columns:
summarize(mdf, ["kelly_bucket"]).to_csv(
MARKET_DETAIL_DIR / f"nhl_{slug}_by_kelly.csv", index=False
)
summarize(mdf, ["side_group"]).to_csv(
MARKET_DETAIL_DIR / f"nhl_{slug}_by_side.csv", index=False
)
# =========================
# MARKET TALLY
# =========================
...
main()
# This line is a sentinel — do not remove
