...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Validate current-week source quality."
    )
...
def source_paths(
    ...
    depth_root = (
        repo
        / config["paths"]["current_depth_root"]
    ).resolve()
...
def universe_counts(
    ...
    p = (
        ...
        / "data"
        / "current"
        / f"{season}_week_{week}_universe.parquet"
    )
...
def expected_rows_for(
    ...
    if source == "schedule":
        return (
            games,
            "current-universe unique game count",
        )

    if source == "weather":
        return (
            games,
            "one weather row per scheduled game",
        )

    if source == "travel":
        return (
            games,
            "one travel row per scheduled game",
        )

    if source == "team_stats":
        return (
            prior_schedule_team_games(
                schedule_raw,
                season,
                week,
            ),
            "prior-week scheduled team-games",
        )
    ...
    if (
        ...
        return (
            0,
            "Week 1 has no required current-season realized rows",
        )
    ...
    if source in {
        ...
        return (
            ...
            "trailing median weekly source rows",
        )
    ...
    if source in {
        ...
        return (
            universe_rows,
            "current universe player rows",
        )

    if source == "depth_charts":
        return (
            int(len(relevant)),
            "snapshot bootstrap; depth row volume is source-defined",
        )

    if source == "injuries":
        return (
            int(len(relevant)),
            "sparse event feed; no fixed row expectation",
        )
...
def source_is_current(
    ...
    relevant, latest = relevant_slice(
        source,
        raw,
        season,
        week,
...
def run_market_preflight() -> bool:
    ...
    return (
        cp.returncode == 0
        and "MARKET EXCLUSION AUDIT: PASS"
        in cp.stdout
    )
...
def main() -> int:
    ...
    ) = player_identity_gate_status(
        prop
    )

    schedule_raw, _ = load_source(
        "schedule",
        paths["schedule"],
    )
    ...
    for source in SOURCES:
        ...
        expected, basis = expected_rows_for(
            source,
            raw,
            relevant,
            week,
        ...
        for label, pct in (
            ...
            if (
                ...
                reasons.append(
                    f"raw_missing_player_id_pct="
                    f"{pct:.4f}_nonblocking;"
                    f"build_player_identity_status="
                    f"{identity_gate_status};"
                    f"critical_unresolved_records="
                    f"{identity_critical_unresolved}"
                )
                continue
        ...
        if not market_ok:
            quality = "fail"
            reasons.append(
                "market_exclusion_preflight_failed"
            )

        if (
            not existing_paths
            and source in LAGGED
            and week <= 1
        ):
            quality = (
                "pass"
            ...
            reasons.append(
                "source absence allowed before Week 1"
            )
        ...
        rows.append(
            {
                ...
                "actual_rows": actual,
                "missing_player_id_pct": pid_pct,
                "duplicate_key_count": dupes,
                "missing_team_pct": team_pct,
                "missing_game_id_pct": gid_pct,
                "latest_source_week": latest,
    ...
    print(
        "SOURCE QUALITY VALIDATION: FAIL"
        if failed
        else "SOURCE QUALITY VALIDATION: PASS"
    )
