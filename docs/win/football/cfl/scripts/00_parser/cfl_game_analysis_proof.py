#!/usr/bin/env python3
"""
CFL GAME ANALYSIS proof parser.

Reads one verified TXT/PDF pair:
...
Writes proof CSVs to:

    docs/win/football/cfl/data_dump/proof/game_analysis/
"""
...
OUT_DIR = Path("docs/win/football/cfl/data_dump/proof/game_analysis")
...
def is_zeroish(values: list[str]) -> bool:
    if not values:
        return False
...
def normalize_values(values: list[str], expected_count: int) -> tuple[list[str], str]:
    if len(values) >= expected_count:
        return values[:expected_count], "OK"

    if is_zeroish(values):
        padded = values + ["0"] * (expected_count - len(values))
        return padded, "OK_ZERO_GP"
...
def write_generic_table(
    ...
    try:
        ...
        blocks = {}
        section_status = f"MISSING_SECTION: {exc}"
    ...
    for team in TEAM_ORDER:
        ...
        if not block:
            ...
            status = section_status if section_status != "OK" else "MISSING_ROW"
        else:
            raw_values = parse_numeric_values(block)
            values, status = normalize_values(raw_values, len(spec.value_columns))
    ...
    write_csv(OUT_DIR / spec.filename, fieldnames, rows)


def parse_top_block(block: str, expected_count: int) -> tuple[str, list[str], str]:
    top_match = re.search(r"\b\d{1,2}:\d{2}\b", block)
    top_value = top_match.group(0) if top_match else ""

    cleaned = block.replace(top_value, " ", 1) if top_value else block
    raw_values = parse_numeric_values(cleaned)
    values, status = normalize_values(raw_values, expected_count)

    return top_value, values, status


def write_time_of_possession_table(text: str, metadata: dict[str, str], pdf_pages: str) -> None:
    value_columns = [
        "time_of_possession",
        "team_possessions",
    ...
    write_csv(
        OUT_DIR / "time_of_possession_field_position.csv",
        BASE_COLUMNS + value_columns + SOURCE_COLUMNS,
        rows,
    )


def write_first_down_tables(text: str, metadata: dict[str, str], pdf_pages: str) -> None:
    try:
        section_lines = extract_section_lines(
            text,
    ...
    for team in TEAM_ORDER:
        ...
        if not block:
            ...
            raw_values = parse_numeric_values(block)
            values, status = normalize_values(raw_values, 16)

        offence_values = values[:6]
        made_values = values[6:16]
...
def main() -> None:
    ...
    for spec in make_specs():
        write_generic_table(text, spec, metadata, pdf_pages)

    write_time_of_possession_table(text, metadata, pdf_pages)
    write_first_down_tables(text, metadata, pdf_pages)

    print(f"proof_dir={OUT_DIR}")
    print("proof_files_written=27")
