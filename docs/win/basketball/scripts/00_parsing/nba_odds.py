...
OUT_DIR = Path("docs/win/basketball/odds/nba")
ERROR_DIR = Path("docs/win/basketball/errors/00_intake")
LOG_FILE = ERROR_DIR / "nba_odds_api.txt"
...
def main() -> int:
    ...
    LOG_FILE.write_text(
        f"=== nba_odds.py RUN {datetime.now().isoformat(timespec='seconds')} ===\n",
        encoding="utf-8",
    )
    ...
    try:
        ...
        log("Fetching NBA events from odds-api.io")
        log(f"Today cutoff date: {today}")

        events = get_nba_events()
        log(f"Found {len(events)} NBA events")
        ...
        for game_date, rows in sorted(grouped.items()):
            out_file = OUT_DIR / f"{game_date}_nba.json"
        ...
        log(
            f"SUCCESS: Wrote {written} NBA odds file(s) | "
