    ...
    for f in files:
        ...
        if not nba_df.empty:
            ...
            def nba_row(row, side):
                ...
                odds = float(row[f"{side}_acceptable_total_american"])
            ...
            for side in ["over", "under"]:
                nba_df[f"{side}_total_juice_odds"] = nba_df.apply(lambda r: nba_row(r, side), axis=1)

            nba_df.to_csv(OUTPUT_DIR / f"{date}_NBA_total.csv", index=False)
        ...
        if not ncaab_df.empty:
            ...
            def ncaab_row(row, side):
                ...
                band = jt[(jt["over_under"] == total) & (jt["side"] == side)]
                if band.empty:
                    return odds
            ...
            for side in ["over", "under"]:
                ncaab_df[f"{side}_total_juice_odds"] = ncaab_df.apply(lambda r: ncaab_row(r, side), axis=1)

            ncaab_df.to_csv(OUTPUT_DIR / f"{date}_NCAAB_total.csv", index=False)
