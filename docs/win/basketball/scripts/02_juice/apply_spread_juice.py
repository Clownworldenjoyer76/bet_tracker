    ...
    for f in files:
        ...
        if not nba_df.empty:
            ...
            for side in ["home", "away"]:
                nba_df[f"{side}_spread_juice_odds"] = nba_df.apply(lambda r: nba_row(r, side), axis=1)

            nba_df.to_csv(OUTPUT_DIR / f"{date}_NBA_spread.csv", index=False)
        ...
        if not ncaab_df.empty:
            ...
            for side in ["home", "away"]:
                ncaab_df[f"{side}_spread_juice_odds"] = ncaab_df.apply(lambda r: ncaab_row(r, side), axis=1)

            ncaab_df.to_csv(OUTPUT_DIR / f"{date}_NCAAB_spread.csv", index=False)
