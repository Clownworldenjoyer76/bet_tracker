...
def compute_moneyline_edges(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["home_prob"] = 1 / pd.to_numeric(df["home_juiced_decimal_moneyline"], errors="coerce")
    df["away_prob"] = 1 / pd.to_numeric(df["away_juiced_decimal_moneyline"], errors="coerce")
...
def compute_puck_line_edges(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["home_prob"] = 1 / pd.to_numeric(df["home_juiced_decimal_puck_line"], errors="coerce")
    df["away_prob"] = 1 / pd.to_numeric(df["away_juiced_decimal_puck_line"], errors="coerce")
...
def compute_total_edges(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["over_prob"] = 1 / pd.to_numeric(df["juiced_total_over_decimal"], errors="coerce")
    df["under_prob"] = 1 / pd.to_numeric(df["juiced_total_under_decimal"], errors="coerce")
