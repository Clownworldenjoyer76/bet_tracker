...
def process_row(df, juice_df, idx, row, file_path):
    ...
    # FIX: was bare except: — now except Exception: so KeyboardInterrupt/SystemExit are not swallowed
    try:
    ...
    if home_extra is None or away_extra is None:
        _log(f"[ROW SKIP] idx={idx} reason=band_lookup_failed home={home_american} away={away_american}")
        return df

    # --- APPLY JUICE IN ODDS SPACE ---
    ...
    if home_juiced_decimal <= 1 or away_juiced_decimal <= 1:
        _log(f"[ROW SKIP] idx={idx} reason=invalid_juiced_decimal home={home_juiced_decimal} away={away_juiced_decimal}")
        return df

    # --- RAW IMPLIED PROB (PRE-NORMALIZATION) ---
...
def main():
    ...
    try:
        ...
        for file_path in files:
            ...
            if df.empty:
                _log(f"[INFO] Empty file — skipping")
                continue
