...
def process_puck_line(df, file_name):
    ...
    home_kelly, h_neg = compute_kelly(df["home_prob_puck_line"], df["home_dk_puck_line_decimal"], file_name)
    away_kelly, a_neg = compute_kelly(df["away_prob_puck_line"], df["away_dk_puck_line_decimal"], file_name)
    ...
    for f in input_files:
        ...
        try:
            ...
            if market == "moneyline":
                ...
                df, neg_kelly = process_puck_line(df, f.name)
            ...
            _log(f"WROTE: {out} ({len(df)} rows, {neg_kelly} kelly values clipped)")
