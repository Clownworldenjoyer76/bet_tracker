...
class NFLOddsScraper(OddsScraper):
    ...
    def _reformat_data(self, df, season):
        ...
        new_df["4thQtr"] = df[7]
        new_df["final"] = df[8]
        new_df["open_odds"] = df[9].apply(lambda x: 0 if x in self.blacklist else x)
        new_df["close_odds"] = df[10].apply(lambda x: 0 if x in self.blacklist else x)
        new_df["close_ml"] = df[11]
        new_df["2H_odds"] = df[12].apply(lambda x: 0 if x in self.blacklist else x)
        return new_df
    ...
    def _to_schema(self, df):
        ...
        for (i1, row), (_, next_row) in self._pairwise(progress):
            if i1 % 2 == 0:
                continue
...
class NHLOddsScraper(OddsScraper):
    ...
    def _reformat_data(self, df, season, covid=False):
        ...
        new_df["date"] = df[0].apply(
            lambda x: self._make_datestr(x, season, start=1, yr_end=3)
            if covid
            else self._make_datestr(x, season)
        )
        new_df["name"] = df[3]
        ...
        new_df["final"] = df[7]
        new_df["open_ml"] = df[8].apply(lambda x: 0 if x in self.blacklist else x)
        new_df["close_ml"] = df[9].apply(lambda x: 0 if x in self.blacklist else x)
...
class MLBOddsScraper(OddsScraper):
    ...
    def _reformat_data(self, df, season):
        ...
        new_df["open_over_under"] = df[19]
        new_df["open_over_under_odds"] = df[20]
        new_df["close_over_under"] = df[21]
        new_df["close_over_under_odds"] = df[22]
        return new_df
