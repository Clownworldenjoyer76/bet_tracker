...
def load_norm_data(league, date_str):
    """Loads betting data using underscore date format: YYYY_mm_DD."""
    # Force underscores for the filename lookup
    standard_date = date_str.replace("-", "_")
    filename = f"norm_dk_{league}_totals_{standard_date}.csv"
...
def process_league(league):
    ...
    for file_path in files:
        ...
        # Load DK betting data (forced to underscores)
        ...
        for gid, rows in games.items():
            # Filter DRAW for Soccer to get Away/Home
            ...
            # Use 'points' for NBA/NCAAB, 'goals' for NHL/SOC
        ...
        if output_rows:
            # Output filename matches your request: clean_{league}_{date_part}.csv
            ...
            print(f"Finished {league}: {out_path.name}")
