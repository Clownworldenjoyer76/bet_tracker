...
NCAAM_FILE = Path("docs/win/basketball/04_select/daily_slate/ncaam_selected.csv")
WNBA_FILE = Path("docs/win/basketball/04_select/daily_slate/wnba_selected.csv")
        ...
        with open(path, "r", encoding="utf-8-sig", newline="") as f:
            ...
            for row in csv.DictReader(f):
                rows.append({
                    ...
                    "line": row.get("bet_line", "") or row.get("line", ""),
                })
            return rows
    except Exception:
...
def main():
    ...
    ncaam_by_date = {}
    for row in read_source(NCAAM_FILE, "NCAAM"):
        d = row["game_date"].replace("-", "_") if "-" in row["game_date"] else row["game_date"]
        ncaam_by_date.setdefault(d, []).append(row)

    wnba_by_date = {}
    for row in read_source(WNBA_FILE, "WNBA"):
        d = row["game_date"].replace("-", "_") if "-" in row["game_date"] else row["game_date"]
        wnba_by_date.setdefault(d, []).append(row)

    # add any dates found only in basketball files
    all_dates |= set(nba_by_date.keys()) | set(ncaam_by_date.keys()) | set(wnba_by_date.keys())
    ...
    for date_str in sorted(all_dates):
        ...
        rows.extend(ncaam_by_date.get(date_str, []))
        rows.extend(wnba_by_date.get(date_str, []))
